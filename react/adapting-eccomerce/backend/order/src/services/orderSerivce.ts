...
export const createOrder = async (
    ...
    try {
        ...
        throw new NotFoundError(
            `address with id ${newOrder.address} not found`
        );
    }
    return await addOrder(newOrder);
};

export const insertProductToOrder = async (
    order_id: string,
    product_id: string,
    quantity: number
) => {
    ...
    const order_product: Omit<order_products, "quantity"> & {
        quantity: number;
    } = {
        order_id,
        product_id,
...
export const deleteProductFromOrder = async (
    ...
    try {
        await removeProductFromOrder(order_id, product_id);
    } catch (err) {
        if (err instanceof PrismaClientKnownRequestError) {
            if (err.code == "p2018") {
                throw new NotFoundError(
                    `product with id: ${product_id} or order with id: ${order_id} not found`
                );
            }
        }
    }
